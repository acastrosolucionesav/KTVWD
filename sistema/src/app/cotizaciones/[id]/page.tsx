    ...
    <div className="max-w-3xl mx-auto p-8 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{c.idTrazabilidad} · {c.familia === 'PUNTUAL' ? NOMBRES_SERVICIO[c.puntual!.servicio] : c.care?.plan}</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <h2 className="text-xs font-bold uppercase tracking-wide text-gray-400 mb-3">Valor total (lo que verá el cliente)</h2>
        <p className="text-3xl font-extrabold text-[#171E27]">{cop(c.totalCliente)}</p>
        <p className="text-xs text-gray-400 mt-1">Sin IVA · sin metros cuadrados (Regla general de visualización)</p>
        {c.puntual?.mostrarInformeInternacional && c.puntual.precioInformeAdicional && (
      ...
      {esGerencia ? (
        <div className="bg-[#171E27] text-white rounded-2xl p-6">
          ...
          <dl className="grid grid-cols-2 gap-y-2 text-sm">
            ...
            <dd className={c.puntual && c.puntual.margenPct! < 0.25 ? 'text-red-400 font-bold' : c.puntual && c.puntual.margenPct! < 0.40 ? 'text-amber-400 font-bold' : 'text-emerald-400 font-bold'}>
              {c.puntual ? (c.puntual.margenPct! * 100).toFixed(1) + '%' : '—'}
            </dd>
          </dl>
        </div>
      ) : (
