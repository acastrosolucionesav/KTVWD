  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className={`${dto.care ? 'max-w-5xl' : 'max-w-4xl'} mx-auto px-6 md:px-12 py-10 space-y-6`}>
          ...
          {p && (
            <>
              {p.itemsLavado.length > 0 ? (
                <>
                  {p.itemsLavado.map((it, idx) => (
                    <div key={idx} className="border border-gray-200 rounded-xl overflow-hidden">
                      <div className="bg-[#66C2F8] text-white px-4 py-2 text-xs font-bold uppercase">{it.nombre}</div>
                      <div className="px-4 py-3 flex justify-between items-center gap-4">
                        ...
                        <span className="font-bold text-[#171E27] shrink-0">{cop(it.precioTotal)}</span>
                      </div>
                    </div>
                  ))}
                  {p.informeBaseNombre && !p.informeBaseCobrado && (
                    <div className="border border-gray-200 rounded-xl overflow-hidden px-4 py-3 text-sm text-gray-700 text-justify bg-[#F7FBFF]">
